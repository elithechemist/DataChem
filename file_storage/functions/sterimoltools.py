...
# TG:
# the values obtained with this script are slightly different from the ones obtained with Molecular Modeling Pro
#
# Problem in the jobtype 1 mode (general sterimol code):
# The original script iterates over all atoms in the molecule, not just the ones attached to atom2 (line 435)
# thus, if atom1 != H, wrong parameters are generated
# function rmv_A_bonded fixes this
#
# Added a warning if both reference atoms are in a ring
...
def bondiRadius(massno):
   ...
   bondi = [0.0,1.09, 1.40, 1.82,2.00,2.00,1.70,1.55,1.52,1.47,1.54,2.27,1.73,2.00,2.10,1.80,1.80,1.75,1.88,2.75,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,1.63,1.40,1.39,1.87,2.00,1.85,1.90,
            ...
            2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,2.00,1.86,0.00]
...
# TG: added construction of connectivity matrix conmat
...
def rmv_A_bonded(atomlist,atomA,atomB,conmat):
    fragment = []
    conmat[atomB,atomB] = 1
    for i in atomlist:
        if conmat[atomB,i] == 1 and i != atomA:
            fragment.append(i)
    #for i in fragment:
    #    atomlist.remove(i)
    lf1,lf2 = 0,1
    while lf1 != lf2: 
        ...
        for j in fragment:
            if j in atomlist:
                atomlist.remove(j)
            if j == atomA or j == atomB:
                continue
            for i in atomlist:
                if i == j or i == atomB:
                    continue
                if conmat[j,i] == 1 and i not in fragment:
                    fragment.append(i)
        lf2 = len(fragment)
    return fragment     
...
class calcSterimol:
   def __init__(self, file, radii, atomA, atomB,verbose,nonterminal,*args):
      stoprings = 0
      confine = 0
      if len(args) != 0:
        stoprings = args[0] 
        confine = args[1]
      self.name = file
      if len(file.split(".com"))>1 or len(file.split(".gjf"))>1: fileData = getinData(file[:-4]) #file.split(".")[0]
      ...
      atomic_co_no,conmat = ncoord(natoms, rcov, atomtype, molcart)     #conmat is needed for both radii types
      if radii == "cpk":
...
# subtract one since the array starts from zero not one
      ...
      if verbose == True:
          ...
          #TG: removed because structures don't look nice like this anymore in python 3. Surely could be adjusted using "".format()
      ...
      if nonterminal:
         atomlist = rmv_A_bonded(atomlist,atomA,atomB,conmat)      # remove nonrelevant atoms from the calculation
      if not nonterminal:
         atomlist.remove(atomA)
      if atomA in atomlist:
         print("\nAtoms a1 and a2 are in cyclic structure, result may not be meaningful\n")
         self.warn = True
         atomlist.remove(atomA)
         if stoprings:
            self.lval, self.B1, self.newB5 = 0,0,0
            return
      else:
         self.warn = False
      ...
      removeatoms = []
      try:
        confinesterimol = confine[0]
        confineradius = confine[1]
        confinevdw = confine[2]
      except:
        confinesterimol = False
        confineradius = 1000
        confinevdw = 1000

      for i in atomlist:
         vect2=np.subtract(getcoords(atomA,molcart),getcoords(i,molcart))
         if confinesterimol and np.linalg.norm(vect2) > confineradius+confinevdw*vdw_radii[i]:
             removeatoms.append(i)
             continue
      ...
      if len(removeatoms) != 0:
        atomlist = [a for a in atomlist if a not in removeatoms]
